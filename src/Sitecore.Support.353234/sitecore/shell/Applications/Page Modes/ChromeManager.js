    ...
    this.setClass = function (renderings, className) {
        ...
        this.getChromesByFieldIdAndDataSource = function (fieldId, dataSourceId) {
            ...
            for (var c = 0; c < chromesList.length; c++) {
                ...
                if ($sc.toShortId(fieldId.toLowerCase()) === fieldIdentifier.toLowerCase() &&
                    $sc.toShortId(dataSourceId.toLowerCase()) === dataSourceIdentifier.toLowerCase()) {
                    result.push(chrome);
                }
            }
        ...
        this.setChromeFocused = function (fieldId, dataSourceId) {
            var chromes = this.getChromesByFieldIdAndDataSource(fieldId, dataSourceId);

            var chrome = chromes[0];
            this.scrollChromeIntoView(chrome);
            this.select(chrome);
        },
        this.setChromesNotValid = function (fieldId, dataSourceId, error, ignoreReadonlyChromes) {
            ...
            for (var c = 0; c < chromes.length; c++) {
                ...
                if (!chrome || (ignoreReadonlyChromes && chrome.isReadOnly())) {
                    continue;
                }
    ...
    */
    this.onChromeUpdated = function (chrome, reason) {
        ...
        if (chrome && chrome.key() == "rendering" && reason == "changevariation") {
            ...
            var component = $sc.first(this.chromes(),
                function () {
    ...
    this.onFieldModified = function (evt, key, value) {
        if (key == "modified") {
            ...
            $sc.each(this.chromes(),
                function () {
                    if (this.key() == "field" &&
                        this.controlId() != modifiedControlId &&
                        this.type.isFieldValueContainer(evt.target)) {
    ...
    this.updateField = function (id, htmlValue, plainValue, preserveInnerContent) {
        var fieldToUpdate = $sc.first(this.chromes(),
            function () { return this.key() == "field" && this.controlId() == id; });
        if (fieldToUpdate) {
            var params = {
    ...
    this.batchChangeVariations = function (combination, selectChrome) {
        ...
        for (var n in combination) {
            ...
            if (isQueueEmpty) {
                var component = $sc.first(Sitecore.PageModes.ChromeManager.chromes(),
                    function () {
    ...
    this.setFieldValue = function (itemUri, fieldID, value) {
        ...
        value = value.replace(/-,scBackslash,-/g, "\\");

        // added additional encoding to prevent text treated as html character entities
